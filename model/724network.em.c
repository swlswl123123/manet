...
int main(int argc, char *argv[])
{
    ...
    {   printf("%d,%lf,%lf,%lf\n", id, lat, lon, alt);
        obj[i + 10] = Ema_Object_Create(model_id, OBJ_NT_NODE_MOBILE);
        i++;
    }
    fclose(file);
    ...
    while (fscanf(file,"%d,%lf,%lf,%lf", &id, &lat, &lon, &alt) != EOF)
    {
        ...
        Ema_Object_Attr_Set(model_id, obj[10+i],
    ...
    while (fscanf(file,"%d,%lf,%lf,%lf", &id, &lat, &lon, &alt) != EOF)
    {
        ...
        Ema_Object_Prom_Attr_Set(model_id, obj[i + 10],
                                 "ar.target altitude", obj[2]);
        Ema_Object_Prom_Attr_Set(model_id, obj[i + 10],
                                 "ar.target latitude", obj[3]);
        Ema_Object_Prom_Attr_Set(model_id, obj[i + 10],
                                 "ar.target longitude", obj[4]);
        Ema_Object_Attr_Set(model_id, obj[i + 10],
