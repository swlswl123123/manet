% function [lat,lon,alt,D,f] = mknet(N,R,lat_0,lon_0,Zmin,Zmax)
    ...
    print(f,'-dpng','-r300',['../doc/NESTED_',num2str(N+1),'_',HASH,'_trj.png']);
    time_step = 165; %s
    save_trj_csv(lat_trj,lon_trj,alt_trj,time_step,['../model/NESTED_',num2str(N+1),'_',HASH,'.trj']);
    lat = [lat;lat_trj(1)];
    lon = [lon;lon_trj(1)];
    alt = [alt;alt_trj(1)];
    csvwrite(['../model/NESTED_',num2str(N+1),'_',HASH,'.csv'],[(1:N+1)',lat,lon,alt]);

    % calculate innet time
    syms k;
    tmp = solve(...
    ((1-k)*X_trj(1) + k*X_trj(2) - X(7)) * ((1-k)*X_trj(1) + k*X_trj(2) - X(7)) + ...
    ((1-k)*Y_trj(1) + k*Y_trj(2) - Y(7)) * ((1-k)*Y_trj(1) + k*Y_trj(2) - Y(7)) + ...
    ((1-k)*Z_trj(1) + k*Z_trj(2) - Z(7)) * ((1-k)*Z_trj(1) + k*Z_trj(2) - Z(7)) == ...
    R*R, k);
